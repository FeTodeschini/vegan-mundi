...
export default function Button({ bgColor, additionalClass, isSubmit="false", type="link", link=null, size="large", children, onClick}: ButtonProps) {
    ...
    if (link === null) {
        return (
            <button type={isSubmit ? "submit" : "button"} className={className} onClick={onClick}>{ children }</button>
        )
    }
    ...
    return <Link href={link}><button type={isSubmit ? "submit" : "button"} className={className}>{ children }</button></Link>
}